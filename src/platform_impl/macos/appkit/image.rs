...
impl NSImage {
    pub fn new_by_referencing_file(path: &NSString) -> Id<Self, Shared> {
        ...
        unsafe { msg_send_id![this, initByReferencingFile: path] }.unwrap()
    }

    pub fn new_with_data(data: &NSString) -> Id<Self, Shared> {
        let this = unsafe { msg_send_id![Self::class(), alloc] };
        unsafe { msg_send_id![this, initWithData: data] }.unwrap()
    }
}
